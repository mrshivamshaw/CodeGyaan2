...
export const getEnrolledCourses = async (req, res) => {
  try {
    ...
    let courseProgressCountArray = await Promise.all(
      enrolledCourses.enrolledCourses.map(async (course) => {
        const completedVideos = await courseProgess.findOne({
          courseId: course._id,
          userId: id,
        });
        ...
        return {
          courseId: course._id,
          courseProgressCount: completedVideos?.completedVideos?.length || 0,
        };
      })
    );
    console.log(courseProgressCountArray);
    return res.status(200).json({
      success: true,
      message: "Enrolled courses fetched successfully",
      data: enrolledCourses.enrolledCourses,
      courseProgressCountArray: courseProgressCountArray,
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
...
export const fetchCourseByCategory = async (req, res) => {
  try {
